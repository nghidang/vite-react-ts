...
```bash
yarn dev        # Dev server (HMR), mode mặc định `development` = env local
yarn dev:dev    # Dev server trỏ vào backend `dev` chung
yarn build      # tsc -b && vite build --mode production
yarn build:dev  # Build cho env `dev` (cũng có build:test / build:prod)
yarn lint       # ESLint
yarn preview    # Preview bản build (kèm preview:dev / preview:test / preview:prod)
yarn test       # Vitest (watch mode)
yarn test:run   # Chạy toàn bộ test 1 lần (CI)
yarn coverage   # Test 1 lần + báo cáo coverage (v8)
yarn test:e2e   # Playwright E2E (test:e2e:ui mở UI runner, test:e2e:report xem report)
```

> **4 môi trường:** `local` / `dev` / `test` / `production`. Vite cấm dùng `local` làm `--mode` nên **local map sang mode `development` mặc định** (`yarn dev`, không cờ `--mode`); 3 môi trường còn lại dùng `--mode <dev|test|production>`. Mỗi mode nạp `.env.<mode>` chồng lên `.env` chung. File `.env.dev/.env.test/.env.production` **được commit** (chỉ chứa config không bí mật như API URL/flag — `VITE_*` lộ ra bundle public). Env `local` chạy bằng `.env` + `.env.local` cá nhân (gitignore qua `*.local`; `cp .env.example .env.local`). Chi tiết ở mục 15 bên dưới.
...
- **Vitest** môi trường `jsdom`, `globals: true` (không cần import `describe`/`it`/`expect`); config nằm dưới key `test` trong `vite.config.ts`, `src/test/setup.ts` đăng ký matcher `jest-dom` và `cleanup()` sau mỗi test.
- Test **colocate** cạnh code (`*.test.ts` / `*.test.tsx`); query theo role/label accessible + `@testing-library/user-event`, không dùng test ID.
- `src/test/README.md` là **bảng catalog**: mỗi loại test một ví dụ tiêu biểu (pure fn, reducer, zod schema, hook, React Query, Redux, Zustand, component, context, form, router, service, single-flight, integration). Helper `queryWrapper.tsx` dựng `QueryClientProvider` cho test hook react-query.
- Type `vitest/globals` + `jest-dom` được nối vào `tsconfig.app.json` nên `tsc -b` cũng type-check luôn test — `vite.config.ts`, `src/test/`, `tsconfig.app.json`.
- **E2E bằng Playwright** (tách riêng Vitest): spec ở `e2e/*.spec.ts`, chạy app thật trên Chromium, mock backend ở tầng mạng bằng `page.route` (không cần server `:3000`); `webServer` tự bật `yarn dev`. Vitest thu hẹp `include` về `src/**` để không "nuốt" file `.spec.ts` của e2e. E2E lộ hành vi **tích hợp** (interceptor 401 → refresh) mà unit test mock-tầng-module không thấy — `playwright.config.ts`, `e2e/login.spec.ts`, `vite.config.ts`.

### 15. Cấu hình môi trường + validate env bằng zod
- **4 môi trường** `local / dev / test / production`: `local` chạy mode `development` mặc định (`yarn dev`), 3 cái còn lại qua `--mode`. Mỗi mode nạp `.env.<mode>` chồng lên `.env` chung; script `dev:dev` / `build:(dev|test|prod)` / `preview:*` trong `package.json`.
- **Validate env bằng zod, fail-fast ngay khi load module**: thiếu/sai biến → ném lỗi rõ ràng thay vì `undefined` mơ hồ ở runtime; `VITE_*` (string) được transform về kiểu thật (vd `VITE_ENABLE_DEVTOOLS` → boolean) — `src/configs/env.config.ts`.
- **Đối tượng `ENV` có type** (`ENV.APP_ENV`, `ENV.API_BASE_URL`, `ENV.IS_PROD`, …) là điểm truy cập config duy nhất — code import từ đây, **không** đọc `import.meta.env` trực tiếp. Thêm biến mới → khai trong `envSchema` **và** `ImportMetaEnv` (`src/vite-env.d.ts`).
- File per-env (`.env.dev/.env.test/.env.production`) **commit** (config không bí mật); `.env.local` cá nhân thì gitignore — template `.env.example`.
...
├── configs/       # env.config (đọc + validate biến môi trường bằng zod → ENV)
...
├── test/          # setup + queryWrapper + README (catalog các loại test); *.test.ts(x) colocate cạnh code
